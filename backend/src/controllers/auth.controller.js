...
/**
 * REGISTER -> create inactive user, send OTP to email
...
async function register(req, res) {
  try {
    ...
    try {
      ...
      console.warn('Failed to send signup OTP email:', mailErr && mailErr.message ? mailErr.message : mailErr);
      // do not fail registration because mail failed; but inform client to help debugging
      return res.status(201).json({ message: 'Account created but failed to send OTP email. Please contact support.' });
    }

    // respond indicating OTP sent
    return res.status(201).json({ message: 'Account created. Please verify using the OTP sent to your email', email: user.email });
  } catch (err) {
...
async function verifyOtp(req, res) {
  try {
    ...
    if (!user.otp_code || !user.otp_expires_at) return res.status(400).json({ message: 'No OTP requested' });
    ...
    await db.query('UPDATE users SET is_active=true, otp_code=NULL, otp_expires_at=NULL WHERE id=$1', [user.id]);
...
async function login(req, res) {
  try {
    ...
    if (!user.is_active) return res.status(403).json({ message: 'Account not verified. Please verify your email.' });
...
async function forgotPassword(req, res) {
  try {
    ...
    try {
      const info = await sendMailSafe(mailOptions);
      if (info && info.previewUrl) console.log('Reset email preview URL:', info.previewUrl);
    } catch (e) {
...
async function resetPassword(req, res) {
  try {
    ...
    const { rows } = await db.query('SELECT * FROM users WHERE email=$1', [email]);
    const user = rows[0];
    if (!user || !user.reset_token || !user.reset_expires_at) return res.status(400).json({ message: 'Invalid or expired token' });

    if (user.reset_token !== token) return res.status(400).json({ message: 'Invalid token' });
    if (new Date(user.reset_expires_at) < new Date()) return res.status(400).json({ message: 'Token expired' });
